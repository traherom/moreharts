package drg.lowlevel;

import java.util.*;
import java.io.*;
import drg.dataflow.BitArraySet;
...
public class Function extends CodeItem
{
	...
	/**
	 * Appends an operation to the current block. Just a shortcut
	 * for getCurrentBlock().appendOper() higher up
	 * @param newOp
	 */
	public void appendToCurrentBlock(Operation newOp)
	{
		currBlock.appendOper(newOp);
	}

	/**
	 * Helper to return the last destination, so we can chain
	 * operations together easily
	 * @return returns last destination operand from the last operand
	 */
	public Operand getLastDest()
	{
		return currBlock.getLastOper().getDestOperand(0);
	}

	/**
	 * Returns a new operation for the current block
	 * @return new operation for current block of function
	 */
	public Operation newOper()
	{
		return new Operation(currBlock);
	}
